...
def load_character_data():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        print("Character data already loaded. skipping.")
    cursor.close()
    conn.close()
...
def load_df_data():
    ...
    cursor.execute("SELECT COUNT(*) FROM devil_fruits")
    if cursor.fetchone()[0] == 0:
        ...
        for df in devil_fruits:
            ...
            try:
                ...
                conn.rollback()
        conn.commit()
        print("✅ Devil Fruit data loaded successfully!")
    else:
        print("Devil Fruit data already loaded. skipping.")
    cursor.close()
    conn.close()
